...
const DEFAULT_TIERS = ['A','B','C','D','E','F'];
const TIER_COLORS = [
	// from A to F
	'#ff6666',
	'#f0a731',
...
window.addEventListener('load', () => {
	...
	document.getElementById('reset-list-input').addEventListener('click', () => {
		if (confirm('Deseja realmente resetar a sua Tierlist?')) {
			soft_reset_list();
		}
	});

	document.getElementById('export-input').addEventListener('click', () => {
		let name = prompt('Dê um nome ao seu arquivo para salvar');
		if (name) {
...
function bind_trash_events() {
	...
	trash.addEventListener('dragenter', (evt) => {
		evt.preventDefault();
		evt.target.src = 'assets/imgs/trash_bin_open.png';
	});
	trash.addEventListener('dragexit', (evt) => {
		evt.preventDefault();
		evt.target.src = 'assets/imgs/trash_bin.png';
	});
